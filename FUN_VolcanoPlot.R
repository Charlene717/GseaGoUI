VolcanoPlot <- function(Marker.df, Pos.List, Neg.List,
                        ...
                        log2FC = 1,PValueSet = 0.05,
                        ShowGeneNum = 5){

    Xintercept = c(-log2FC, log2FC)
    Yintercept = -log10(PValueSet)
    #
    library(ggplot2)
    library(cowplot)
    ...
    Marker.df2$PValue <- Marker.df2$PValue+1.0e-300

    Marker.df2$color <- ifelse(Marker.df2$PValue< PValueSet & abs(Marker.df2$logFC)>= log2FC,ifelse(Marker.df2$logFC > log2FC,'red','blue'),'gray')
    ...
    library(ggrepel)
    VolcanoPlot <- ggplot(Marker.df2, aes(logFC, -log10(PValue), label = genelabels, col = color)) +
      geom_point(size = 3) +
      theme_bw() +
